...
def print_timeline(timeline):
    ...
    for seg in timeline.segments:
        ...
        if seg.audio:
            print(f"  silence_ratio: {seg.audio.silence_ratio:.2f}  "
...
def process_video(video_path, frames_dir=None, audio_path=None, timeline_path=None):
    frames_dir = frames_dir or os.path.join(OUT_DIR, "frames")
    audio_path = audio_path or os.path.join(OUT_DIR, "audio.wav")
    timeline_path = timeline_path or os.path.join(OUT_DIR, "timeline.json")

    if os.path.exists(timeline_path):
        print(f"[cache] found existing timeline at {timeline_path}, loading it instead of reprocessing")
    ...
    print("[6/6] building timeline and generating audio-description narration for gaps ...")
    ...
    print(f"      {ad_eligible_count}/{len(timeline.segments)} segments are ad_eligible, generating narration ...")
...
if __name__ == "__main__":
    timeline = process_video(os.path.join(IN_DIR, "test.mp4"))
