...
async def run_pipeline(
    ...
    before narration), ``narration`` (per-segment AD text), and
    ``narration_audio`` (per-segment synthesized clip: filename, duration,
    overflow). Frame and narration events arrive out of order — consumers must
    key off ``shot_id``/``index`` and ``segment_id``.
    """
    ...
    logger.info("stage 1/7 segmentation: detecting shots + sampling frames")
    ...
    logger.info(
        "stage 1/7 segmentation done: %d shot(s), %d frame(s) in %.1fs",
        len(shots),
        frame_count,
        time.monotonic() - t0,
    )
    ...
    logger.info("stage 2/7 vision: describing %d frame(s) via Gemini", frame_count)
