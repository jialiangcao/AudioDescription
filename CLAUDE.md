...
drawn from the shot's whole frame sequence — and synthesizes that line to speech with Kokoro.
...
analysis beside it, grouped by shot, with the shot's narration underneath), and the Q&A box.
...
narration text, per-segment narration audio); CPU-bound stages run via `asyncio.to_thread`,
Gemini stages run natively async.
...
- Per-job temp dirs under `<tempdir>/adesc-jobs/<job_id>/` hold the uploaded `source.<ext>`, the
  ...
  index within the shot), `narration/shot_XXXX.wav` synthesized narration clips, and a mirrored
  `status.json` for crash recovery. They are swept after `JOB_TTL_SEC` (kept until then so Q&A can
  read the frames and the frontend can serve the frame-by-frame view and play the narration clips).
  The old fixed `src/in`/`src/out` layout is gone.
